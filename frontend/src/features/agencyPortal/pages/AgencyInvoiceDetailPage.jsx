...
export const AgencyInvoiceDetailPage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchInvoiceDetail = async () => {
      try {
        const result = await agencyPortalService.getInvoiceDetail(id);
        setData(result.data);
      } catch (err) {
  ...
  const handleDownloadPDF = () => {
    ...
    window.open(`${apiBaseUrl}/agency-portal/invoices/${id}/pdf`, '_blank');
  };
  ...
  const isUnpaid = ['sent', 'partially_paid', 'overdue'].includes(invoice.Status?.toLowerCase());
    ...
    <div className="space-y-6 text-left max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <button
          ...
          className="font-bold border border-slate-200 text-slate-700 hover:bg-slate-50 flex items-center gap-1.5 self-end sm:self-center text-xs py-2 px-4 rounded-2xl transition-all bg-white shadow-2xs"
        >
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold text-slate-600">
              ...
              <div className="space-y-2.5 border-t border-slate-100 sm:border-t-0 sm:pl-4">
                ...
                <div className="flex items-center gap-4 text-slate-500 text-[11px] pl-5.5">
                  ...
                  <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-slate-400" />{formatTime(invoice.ScheduledStart)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-xs">
            <div className="px-6 py-4 bg-slate-50/50 border-b border-slate-100">
              <h3 className="text-xs font-black text-slate-755 uppercase tracking-widest">
                Line Items
              </h3>
            </div>
            ...
            <table className="w-full text-left text-xs font-semibold text-slate-600">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200/60 text-[10px] font-bold text-slate-400 uppercase tracking-wider select-none">
                  ...
                  <th className="px-6 py-2.5 text-right">Quantity / Hrs</th>
                  <th className="px-6 py-2.5 text-right">Rate</th>
                  <th className="px-6 py-2.5 text-right">Amount</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-slate-605">
                {/* Base clean */}
                <tr className="hover:bg-slate-50/20">
                  <td className="px-6 py-3.5 font-bold text-slate-800">
                    Cleaning Service (Base Rate)
                  </td>
                  <td className="px-6 py-3.5 text-right">{activeHours}h</td>
                  <td className="px-6 py-3.5 text-right">{formatGBP(activeRate)}</td>
                  <td className="px-6 py-3.5 text-right font-bold text-slate-800">{formatGBP(invoice.SubTotal)}</td>
                </tr>
                ...
                {options && options.map((opt, i) => (
                  <tr key={i} className="hover:bg-slate-50/20 text-slate-500">
                    ...
                    <td className="px-6 py-3.5 text-right">1</td>
                    <td className="px-6 py-3.5 text-right">{formatGBP(opt.Fee)}</td>
                    <td className="px-6 py-3.5 text-right font-bold text-slate-800">{formatGBP(opt.Fee)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-xs text-left space-y-3.5">
            ...
            <div className="space-y-2 bg-slate-50 p-3 rounded-2xl border border-slate-100 text-[11px] font-semibold text-slate-650">
              <div>
                ...
                <span className="text-slate-800 font-bold block mt-0.5 font-sans">Barclays Bank</span>
              </div>
              <div>
            ...
            <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex gap-2 text-[10px] text-slate-550 font-semibold leading-relaxed">
...
export default AgencyInvoiceDetailPage;
